...
SELECT  (CASE WHEN col.colorder = 1 THEN obj.name ELSE '' END) AS 表名,
        ...
        (CASE WHEN ISNULL(COLUMNPROPERTY(col.id, col.name, 'Scale'), 0) >0 then 
        t.name+'('+convert(varchar(10), col.length)
        +','+ convert(varchar(10),ISNULL(COLUMNPROPERTY(col.id, col.name, 'Scale'), ''))+')' else 
        t.name+'('+convert(varchar(10), col.length)+')'  end )daf,
        t.name AS 数据类型 ,
        col.length AS 长度 ,
        ...
        (CASE WHEN EXISTS 
			( SELECT   1
           FROM     dbo.sysindexes si INNER JOIN dbo.sysindexkeys sik ON si.id = sik.id AND si.indid = sik.indid
